...
func Test_Fragment_Assemble(t *testing.T) {
    ...
    for i:=0; i < 3; i++ {
        ...
        frags := hf.bufFragmentate(c, frame)
    ...
    rpacks, fails := hf.assemble(packets, []*HopPacket{})
    ...
    rpacks, fails = hf.assemble(first_packets, []*HopPacket{})
    ...
    second_packets := append(fails, packets[len(packets)-2:]...)
    rpacks, fails = hf.assemble(second_packets, fails)
